...
| Missing or incompatible interface | Review is requested, but both bundled-client access and a compatible host tool are unavailable. | Report incomplete review and retain records; do not infer absent direct API access merely from an absent MCP. Complete the base workflow without requesting secrets or changing permissions. |
